...
function approveRequest() {
  axios.put(`/transactions/approve/${ props.transaction.id }`).then(
    () => {
...
function declineRequest() {
  axios.put(`/transactions/decline/${ props.transaction.id }`).then(
    () => {
...
<template>
  ...
  >
    ...
    <td class="px-6 py-4">
      <span v-if="props.transaction.status === 'Approved'" class=" text-blue-500 font-bold">Approved</span>
      <span v-if="props.transaction.status === 'Pending'" class=" text-orange-400 font-bold">Pending</span>
      <span v-if="props.transaction.status === 'Declined'" class=" text-red-500 font-bold">Declined</span>
    </td>
