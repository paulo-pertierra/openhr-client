...
function validatePayload() {
  if (!username.value || !password.value) { Swal.fire("Empty fields", "Please fill out the required fields", "warning"); return }
  loginUser();
}
...
async function loginUser() {
  await axios.post("/auth/login", payload)
    .then(async (res) => {
      ...
      if (role === "employee" || role === "intern") { router.push("/user"); return; }
      throw new Error
    })
    .catch((error) => {
      console.log(error);
...
<template>
  <form id="signin-form" class="space-y-4 md:space-y-6" @submit.prevent>
    ...
    <p class="text-sm font-light text-gray-500 dark:text-gray-400">
      ...
      <RouterLink to="signup" class="font-medium text-blue-600 hover:underline dark:text-blue-500">Sign up</RouterLink>
    </p>
  </form>
</template>
